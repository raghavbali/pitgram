...
let runnersPatchedPromise: Promise<void> | undefined;

function ensureRunnersPatched(): Promise<void> {
	if (runnersPatchedPromise) return runnersPatchedPromise;
		...
		if (typeof require !== "undefined" && require.cache) {
			for (const key of Object.keys(require.cache)) {
				try {
					...
					if (!exp) continue;
					if (exp.ExtensionRunner) patchExtensionRunnerClass(exp.ExtensionRunner);
					if (exp.default && exp.default.ExtensionRunner) patchExtensionRunnerClass(exp.default.ExtensionRunner);
				} catch {}
			}
		}
	})();
...
interface RelayResult<T = unknown> {
	ok: boolean;
	error?: string;
	turn?: T | null;
	pending?: number;
...
export function tgcloudAppId(token: string): string | undefined {
...
export async function getConfiguredModels(ctx: ExtensionContext): Promise<any[]> {
	const modelsJsonPath = join(homedir(), ".pi", "agent", "models.json");
	try {
		const content = await readFile(modelsJsonPath, "utf8");
		const parsed = JSON.parse(content) as ModelsJsonFile;
		if (parsed.providers) {
			...
			for (const [provider, config] of Object.entries(parsed.providers)) {
				if (Array.isArray(config.models)) {
					for (const mDef of config.models) {
						...
						const modelObj = match || allModels.find((m: any) => m.provider === provider && m.id === mDef.id);
						if (modelObj) {
...
export default function (pi: ExtensionAPI) {
	...
	async function loadRelaySources(): Promise<Record<string, string>> {
	...
	async function callRelay<T = unknown>(args: Record<string, unknown>, signal?: AbortSignal): Promise<RelayResult<T>> {
		if (!config.relayToken) throw new Error("Pitgram relay is not configured");
		const appId = tgcloudAppId(config.relayToken);
		if (!appId) throw new Error("Invalid Telegram Serverless CLI token");
		...
		if (!result || typeof result.ok !== "boolean") throw new Error("Invalid response from Telegram Serverless relay");
		if (!result.ok) throw new Error(result.error || "Relay operation failed");
		return result;
	}
		...
		if (lower.startsWith("/model")) {
			...
			if (!arg) {
				try {
					...
					const lines = models.map((model: any, index: number) => {
				...
				try {
					...
					if (!isNaN(index) && index > 0 && index <= models.length) {
						...
						targetModel = models.find((m: any) => 
							`${m.provider}/${m.id}`.toLowerCase() === lowerArg ||
			...
			if (!arg) {
				...
				if (!validLevels.includes(arg)) {
					await sendTextReply(
						firstMessage.chat.id,
						firstMessage.message_id,
						`Invalid thinking level: ${arg}. Valid levels: ${validLevels.join(", ")}`
					);
					return;
				}
				try {
